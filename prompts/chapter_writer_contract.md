...
仅用于 `llm_narrative` 章节。worker pack 已包含本 worker 所需工程事实、研究证据、章节结构与长度预算。

## 规则

1. 只能使用 worker pack 中的工程事实和 research evidence；不得新增工程数字、市场事实或经济参数。
...
5. 输出仅需：`section_id / draft_status / subsections / source_evidence_ids`。`claims`、`open_items` 为历史兼容字段，不要求生成。
...
输出须满足 `schemas/section_drafts.schema.json`。
